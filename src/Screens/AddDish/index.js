...
import { dishRegister } from "../../Redux/Reducers/Dishes/addDishActions";
...
class AddDishs extends React.Component {
  ...
  async addDish(
    ...
    if (dish_name.length == 0) {
      this.setState({ showDishNameError: true, isLoading: false });
      alert("Le nom du plat est obligatoire");
      return;
    } else this.setState({ showDishNameError: false });
    if (price.length == 0) {
      alert("Le prix du plat est obligatoire");

      this.setState({ showDishPriceError: true, isLoading: false });
      return;
    } else this.setState({ showDishPriceError: false });

    var formdata = new Object();
    this.state.specialItemName.forEach((element) => {
      this.setState({ _specialItemName: element });
    });
    for (let i = 0; i < this.state.tags.tagsArray.length; i++)
      this.setState({ Tags: this.state.tags.tagsArray[i] });
    // formdata = {
    //   dish_name: "Zinger Shwarma",
    //   dish_description: "Fast Food",
    //   dish_images: ["https://firebasestorage.googleapis.com/v0/b/cafe-perks.appspot.com/o/IMG_0005.JPG?alt=media&token=78914790-7996-4b08-82a2-8f8f0fb7db2c"],
    //   dish_type: "5f59a6ffba0e5511ec1000a8",
    //   cuisine_type: this.state.cuisineTypeName
    //     ? this.state.cuisineTypeName.value
    //     : null,
    //   special_item: "5f71b4085b71911561f3032c",
    //   distance: "300",
    //   price: "6",
    //   resturant_id: "60dc12f10111be51200cac56",
    ...
    // try {
      ...
      // try {
      //   this.props.dishRegister(formdata, (res) => {

      //     console.log("dish added Successfully ",res)
    ...
    //////
    // const response = await fetch(`${host}/dishs/register`, {
    //   method: "POST",
    //   headers: {
    ...
    // })
      ...
      //   if (responseJson != undefined && responseJson.status == 200) {
      //     alert("Le plat a été ajouté avec succès.");
      ...
      //   alert("problème de connexion Internet" + error.message);
      // });
  }
  async addDishes () {
    formdata = {
